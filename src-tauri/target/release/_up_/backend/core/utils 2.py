...
class ConfigLoader:
    def __init__(self, config_path: str = "./config/config.yaml"):
        # Check for environment variable override (for Electron app)
        if os.environ.get('GMAPS_CONFIG_PATH'):
            self.config_path = os.environ.get('GMAPS_CONFIG_PATH')
        else:
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        try:
            if not os.path.exists(self.config_path):
                # Try to find example config in the same directory
                ...
                if os.path.exists(example_path):
                    logger.warning(f"Config file not found. Using example config from {example_path}")
